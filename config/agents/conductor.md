...
1. **Question / no code change** - answer it. Use @explore for codebase
   ...
   small. Do not speculate about code you have not seen; delegate the
   lookup. Brief them to start with `kopipasta map --json <path>`
   (skill: `codebase-map`) - a free symbol skeleton - and to return paths
   plus conclusions, never file dumps.
