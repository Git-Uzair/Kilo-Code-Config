...
# EU inference profile, same region pin as the Opus agents (kilo.jsonc). If
# EU ever errors on capacity/availability, the verified fallback is
# model: amazon-bedrock/global.anthropic.claude-fable-5 - present in
# `kilo models`; no other change needed.
# Bring-up status 2026-08-21: config verified live (agent loads, model
# resolves, variant "max" recorded on the session) but BOTH fable-5
# inference profiles are IAM-denied for the colleague-bedrock key
# (no bedrock:InvokeModelWithResponseStream on them), and the direct
# anthropic/claude-fable-5 rollback 404s (no Fable access on that org).
# Fix is AWS-side policy, not config.
